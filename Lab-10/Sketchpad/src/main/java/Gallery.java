...
Ans: I left Shape class abstract. It would never be constructed
     as it cannnot be define itself how much area it should take.
     Again in cavas there are no methods to draw "Shape".
...
Ans: I left ShapeException and Rectangle concrete.
    ...
    need to call ShapeException which is better
     than calling both its child class using pipe operator(it is a bit reduntdant).
3. One method you overrode even though the parent implemented it, and why.
Ans: I overrode setwidth() method in Square eventhough Rectangle implemented it.
...
Ans: I did not overried in area() in Square as we can already implement it correctly
